...
const Livre = () => {
  ...
  const [searchTerm, setSearchTerm] = useState(""); // Option for search
  const [selectedOption, setSelectedOption] = useState(null); // Option for PDF
  ...
  const save = (id) => {
    ...
    if (id) {
      ...
      axios
        .put(`${REST_API_BASE_URL}/${id}`, livre)
      ...
      axios
        .post(`${REST_API_BASE_URL}/ajouter`, livre)
        .then(() => {
  ...
  // Filter books based on selected option
  const filteredLivres =
    selectedOption === "Tous"
      ? livres
      : livres.filter((livre) => livre.annee === parseInt(selectedOption));
    ...
    <>
      ...
      <div className="container ">
        ...
        <div className="row mb-1 justify-content-between">
          <div className="col-auto">
            ...
            <Button
              type="success"
              onClick={() => OpenPrint()}
              style={{ background: "MediumSeaGreen", gap: "4px" }}
            >
        ...
        <Modal
          ...
          cancelButtonProps={{
            style: { backgroundColor: "white", color: "red", border: "dashed" },
          }}
        >
          {" "}
          <br />
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item
                  ...
                  name="nom_livre"
                  rules={[
                    { required: true, message: "Entrer le nom de livre!" },
                  ]}
                >
                  <Input
                    value={nom_livre}
                    onChange={(e) => setNom_livre(e.target.value)}
                  />
                </Form.Item>
              </Col>
              <Col span={12}>
                <Form.Item
                  ...
                  name="nom_auteur"
                  rules={[
                    { required: true, message: "Entrer le nom d'auteur!" },
                  ]}
                >
                  <Input
                    value={nom_auteur}
                    onChange={(e) => setNom_auteur(e.target.value)}
                  />
                </Form.Item>
              </Col>
            </Row>
            ...
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item
                  ...
                  name="livre_code"
                  rules={[
                    { required: true, message: "Entrer le code du livre!" },
                  ]}
                >
                  <Input
                    value={livre_code}
                    onChange={(e) => setLivre_code(e.target.value)}
                  />
                </Form.Item>
              </Col>
              <Col span={12}>
        ...
        <div className="livre-container">
                ...
                <div className="livre-hover-info">
                  ...
                  <div className="button-container">
                    <button
                      ...
                      onClick={() => supprimer(livre.id_livre)}
                    >
                      <i className="fa fa-trash-o"></i>
                    </button>
                    <button
                      className="btn btn-warning"
                      onClick={() => showModal(livre.id_livre)}
                    >
          ...
          <Modal
            title={
              <div style={{ textAlign: "center", color: "#22313f" }}>
                Selectionner l'option que vous souhaitez d'imprimer{" "}
              </div>
            }
                      ...
                      rules={[
                        {
                          required: true,
                          message:
                            "Entrer le code du livreSelectionner un choix",
                        },
                      ]}
                    >
          ...
          {selectedOption && (
            <PDFDownloadLink
              ...
              fileName={`livres_${selectedOption}.pdf`}
            >
              {({ loading }) => (
